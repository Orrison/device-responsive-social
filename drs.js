...
$fbUserID = 423992034621484;
    
if (isIphone || isIpad) {
    $('a[href*="facebook.com"]').on('click', function(e){
        e.preventDefault;
        $fUser = $('a[href*="facebook.com"]').attr("href").match(/facebook.com\/([a-zA-Z_.-]+)/)[1];
    ...
    $('a[href*="instagram.com"]').on('click', function(e){
        e.preventDefault;
        $iUser = $(this).attr("href").match(/instagram.com\/([a-zA-Z_.-]+)/)[1];
        var now = new Date().valueOf();
        setTimeout(function () {
        if (new Date().valueOf() - now > 100) return;
    ...
    $('a[href*="twitter.com"]').on('click', function(e){
        e.preventDefault;
        $tUser = $(this).attr("href").match(/twitter.com\/([a-zA-Z_.-]+)/)[1];
        var now = new Date().valueOf();
        setTimeout(function () {
        if (new Date().valueOf() - now > 100) return;
    ...
    $('a[href*="facebook.com"]').on('click', function(e){
        e.preventDefault();
        $fUser = $(this).attr("href").match(/facebook.com\/([a-zA-Z_.-]+)/)[1];
        var now = new Date().valueOf();
        setTimeout(function () {
        if (new Date().valueOf() - now > 100) return;
            window.location = "https://www.facebook.com/" + $fUser;
